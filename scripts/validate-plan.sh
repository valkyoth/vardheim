...
grep -q 'Durable composed EAB account creation through staged identity, fingerprints, finalized request aggregate, consuming `PublicationAttempt`, qualified store commit, and evidence-gated abandonment' "$plan"
...
grep -q 'Durable request-image representation and recovery validation contract' "$plan" && grep -q 'Extensible outbox publication and qualified-commit contract' "$plan" && grep -q 'PublicationCommitUnknown' "$plan"
...
grep -q 'Reusable composed EAB conformance through target/metadata, locally verified and unsplit finalized states, consuming publication attempt, extensible-store qualification, evidence-gated abandonment, and quarantine milestones' "$plan"
