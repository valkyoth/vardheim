...
grep -q 'Transactional durable identity allocator/store boundary for reservation, abandonment, republication, and outbox commit' "$plan"
...
grep -q 'Durable composed EAB account creation through staged identity, fingerprints, finalized request aggregate, consuming `PublicationAttempt`, exact republication, qualified store commit, and evidence-gated abandonment' "$plan"
...
grep -q 'Interrupted signed-request abandonment, republication, and replacement contract' "$plan"
...
grep -q 'Durable request-image representation and recovery validation contract' "$plan" && grep -q 'Extensible outbox publication and qualified-commit contract' "$plan" && grep -q 'PublicationCommitUnknown' "$plan" && grep -q 'RepublishPermit' "$plan"
...
grep -q 'Reusable composed EAB conformance through target/metadata, locally verified and unsplit finalized states, consuming publication attempt, exact single-use republication, extensible-store qualification, evidence-gated abandonment, and quarantine milestones' "$plan"
...
grep -q 'Reusable adapter conformance for transport, signer, public stores, clock, entropy, staged/abandoned identity.*consuming publication attempts' "$plan"
