...
for package in vardheim-core vardheim-challenge-http vardheim-challenge-dns vardheim-challenge-tls; do
    cargo package -p "$package" --allow-dirty --no-verify --offline
done

cargo package -p vardheim --allow-dirty --no-verify --offline \
...
for archive in target/package/vardheim-*.crate; do
    if tar -tf "$archive" | rg -q '(^|/)(rfc/|\.github/)|\.(webp|png|jpe?g|gif|svg)$'; then
