...
| Security-sensitive IDs | issuer separates internal stable, ephemeral correlation and caller idempotency identities; admitted entropy/counter source domain-separated by tenant/purpose/store/provider/node/incarnation/recovery epoch; collision/exhaustion, fork/snapshot/restore and multi-node safe; persist before external use |
...
| Restored stores | rollback-protected/detecting/unprotected capability; external authenticated witness binds store/state head to monotonic counter/journal and models commit skew, loss/reset/clone/split-brain/key rotation; epochs or operator declaration invalidate authority/leases/fences/sessions and quarantine restored state; high assurance rejects unprotected storage without explicit reduced-assurance selection |
| Executor boundary | generic executors are local-process only and never serialize live authority; each later remote signer/DNS/KMS/deployment/agent protocol is purpose-specific, bounded, versioned, mutually authenticated, replay/downgrade protected, recovery-bound and returns observations only |
